...
public interface RedisCacheExt {
     ...
     */
    void msetEx(Map<String, String> map, long expireSeconds);
     ...
     */
    List<Object> pipelined(Consumer<RedisCacheAsync> doInPipelined);
     ...
     * @return 锁结果, 使用isSuccess判断是否成功获取锁, 调用close释放锁
     */
    LockState tryLock(long waitMilliseconds, long expireSeconds, String... keys);
     ...
     * @return 锁对象, 使用lock方法开始获取锁
     */
    Lock newLock(long waitMilliseconds, long expireSeconds, String... keys);

    /**
     * 内部RedisCachePool实例
     */
    RedisConnectionPool getRedisCachePool();
}